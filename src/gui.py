...
class App(ctk.CTk):
    ...
    def _build_backup_tab(self, tab):
        ...
        card.grid(row=0, column=0, sticky="ew", pady=(6, 6))
        card.grid_columnconfigure(0, weight=1)
        pad = {"padx": 8}

        ev = ctk.CTkFrame(card, fg_color="transparent"); ev.grid(row=0, column=0, sticky="ew", pady=(8, 0), **pad)
        self._cap(ev, "Your email address").pack(anchor="w", pady=(0, 4))
        self.user = self._entry(ev); self.user.pack(fill="x")

        pwv = ctk.CTkFrame(card, fg_color="transparent"); pwv.grid(row=1, column=0, sticky="ew", pady=(10, 0), **pad)
        self._cap(pwv, "Your password   (or an app password, if 2-step verification is on)").pack(anchor="w", pady=(0, 4))
        prow = ctk.CTkFrame(pwv, fg_color="transparent"); prow.pack(fill="x"); prow.grid_columnconfigure(0, weight=1)
        self.pw = self._entry(prow, show="•"); self.pw.grid(row=0, column=0, sticky="ew")
        ...
        dv = ctk.CTkFrame(card, fg_color="transparent"); dv.grid(row=2, column=0, sticky="ew", pady=(10, 0), **pad)
        self._cap(dv, "Save backup to").pack(anchor="w", pady=(0, 4))
        drow = ctk.CTkFrame(dv, fg_color="transparent"); drow.pack(fill="x"); drow.grid_columnconfigure(0, weight=1)
        ...
        df = ctk.CTkFrame(card, fg_color=LIGHT_GREY, corner_radius=10); df.grid(row=3, column=0, sticky="ew", pady=(12, 0), **pad)
        dfin = ctk.CTkFrame(df, fg_color="transparent"); dfin.pack(fill="x", padx=12, pady=10)
        ...
        ctk.CTkLabel(avin, text="Drive & anything else  (optional)", font=self.f_label, text_color=TEXT).pack(anchor="w")
        ctk.CTkLabel(avin, text="Drive isn't live. In webmail: Settings → Download your personal data → tick Drive → "
                     "download the .zip, then add it here.", font=self.f_label, text_color=MUTED,
        ...
        self.insecure = ctk.CTkCheckBox(card, text="Skip certificate check (only if you get a TLS/certificate error)",
                                        font=self.f_label, text_color=MUTED, fg_color=TEAL, hover_color=TEAL_HOVER,
                                        variable=self.insecure_var)
        self.insecure.grid(row=5, column=0, sticky="w", pady=(12, 0), **pad)
    ...
    def _build_restore_tab(self, tab):
        tab.grid_columnconfigure(0, weight=1)
        card = ctk.CTkFrame(tab, fg_color=WHITE, corner_radius=10); card.grid(row=0, column=0, sticky="ew", pady=(6, 6))
        card.grid_columnconfigure(0, weight=1); pad = {"padx": 8}

        ctk.CTkLabel(card, text="Upload a backup folder (made by the Backup tab) into a Gmail / Workspace account.",
                     font=self.f_label, text_color=MUTED, wraplength=640, justify="left").grid(row=0, column=0, sticky="w", pady=(10, 4), **pad)

        # Step 1: Google Secure Login
        login_frame = ctk.CTkFrame(card, fg_color=LIGHT_GREY, corner_radius=10)
        login_frame.grid(row=1, column=0, sticky="ew", pady=(10, 0), **pad)
        login_inner = ctk.CTkFrame(login_frame, fg_color="transparent")
        login_inner.pack(fill="x", padx=12, pady=12)
        
        self.google_login_btn = ctk.CTkButton(login_inner, text="Sign in with Google", width=180, height=36, corner_radius=8, font=self.f_btn,
                                              fg_color=TEAL, hover_color=TEAL_HOVER, text_color=WHITE,
                                              command=self.run_google_login)
        self.google_login_btn.pack(side="left")
        
        self.google_status_lbl = ctk.CTkLabel(login_inner, text="Not signed in.", font=self.f_label, text_color=MUTED)
        self.google_status_lbl.pack(side="left", padx=(15, 0))

        # Checkbox for skip certificate check on Restore tab
        self.insecure_r = ctk.CTkCheckBox(card, text="Skip certificate check (only if you get a TLS/certificate error)",
                                          font=self.f_label, text_color=MUTED, fg_color=TEAL, hover_color=TEAL_HOVER,
                                          variable=self.insecure_var)
        self.insecure_r.grid(row=2, column=0, sticky="w", pady=(12, 0), **pad)

        ev = ctk.CTkFrame(card, fg_color="transparent"); ev.grid(row=3, column=0, sticky="ew", pady=(12, 0), **pad)
        self._cap(ev, "Verified Gmail / Workspace address").pack(anchor="w", pady=(0, 4))
        self.r_user = self._entry(ev); self.r_user.pack(fill="x")
        self.r_user.configure(placeholder_text="Will fill in automatically after Google Login", state="disabled")

        sv = ctk.CTkFrame(card, fg_color="transparent"); sv.grid(row=4, column=0, sticky="ew", pady=(10, 0), **pad)
        self._cap(sv, "Backup folder to upload").pack(anchor="w", pady=(0, 4))
        srow = ctk.CTkFrame(sv, fg_color="transparent"); srow.pack(fill="x"); srow.grid_columnconfigure(0, weight=1)
        self.r_src = self._entry(srow); self.r_src.grid(row=0, column=0, sticky="ew")
        ...
        self.r_btn = ctk.CTkButton(card, text="Upload to Gmail", height=42, corner_radius=8, font=self.f_btn,
                                   fg_color=TEAL, hover_color=TEAL_HOVER, text_color=WHITE, command=self.run_restore)
        self.r_btn.grid(row=5, column=0, sticky="ew", pady=(16, 12), **pad)
